...
* 指定した時間をターゲットとしてカウントダウン/カウントアップをします。
* 24時間以上の場合も、日ではなく時間として表示されます。（例：29:12:20）

![Alt text](../images/import_use_p02.png)

* タグは `live.time.countDown`、`live.time.countUp` になります。
...
|シーン切替|OBSのシーンを指定のものに切り替えます|
|ソース|OBSの指定ソースを操作します（表示・非表示どちらか選べます）|
|再生終了後に非表示|ソースがメディアの場合、再生が終わったら非表示にします|
|再生終了時にアイテム非表示|再生が終わったらアイテムを非表示にします|
|ブラウザソース>ソース|OBSのブラウザの表示内容を個別に指示します。タグが利用可能です。|
|ブラウザソース>ファイル|OBSのブラウザのファイルを差し替えます。タグが利用可能です。|
|ブラウザソース>スタイルシート|OBSのブラウザのスタイルシートを差し替えます。タグが利用可能です。|
...
|VTube Studio|VTube Studio にキーバインドを実行するよう指示をだします。キーにはキーバインドの設定名（タイトル）をいれます|
...
![Alt text](../images/import_use_p18.png)
